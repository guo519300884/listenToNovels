# 发布说明

## 立刻用上 10 倍速（绿色版）

若当前还是 `1.0x / 1.5x / … / 5.0x` 那套按钮，说明仍在用旧程序。

1. 下载本目录的 **`听小说_10倍速_直接覆盖用.zip`**
2. 退出正在运行的听小说
3. 解压后**整夹覆盖**本机 `PC_TS\听小说应用\`
4. 只双击其中的 `听小说.exe`
5. 左上角应显示 **`v1.1.0 · 最高10x`**，速度键为 `1x 2x 3x 5x 8x 10x`

也可在项目根目录双击 `立即更新到10倍速.bat`（需 Git）。

---

## 发给别人

把 `发布\` 里对应版本的安装包发出去即可，例如：

**`听小说安装包_v1.2.0.exe`**

对方双击 → 选目录 → 完成安装。

## 自己重新打包

### 1. 先打主程序（有更新时）

```bat
novel_listener\build_exe.bat
```

把生成的 `dist\听小说.exe` 放到 `听小说应用\听小说.exe`。

> 当前绿色版也可直接双击 `听小说应用\听小说.exe`（启动器 + `runtime_pack.zip`），已含最高 10 倍速与调速立即生效；用上面脚本打出的单文件 exe 会覆盖启动器。

### 2. 再打安装包（自定义版本）

在项目**根目录**（若在 PowerShell 中，必须加 `.\`）：

```powershell
.\build_installer.bat 1.2.0
```

或双击 `build_installer.bat`，按提示输入版本号。

### 3. 输出

```text
发布\听小说安装包_v1.2.0.exe
```

## 版本号

- 格式：`1.0.0` 或 `1.2.3.4`
- 成功后写入根目录 `VERSION`，下次默认用它
- 也可先改 `VERSION` 文件，再直接运行 `build_installer.bat` 回车确认

## 常见问题

| 现象 | 处理 |
|------|------|
| 找不到听小说.exe | 先运行 `novel_listener\build_exe.bat` |
| 找不到 ISCC.exe | `winget install JRSoftware.InnoSetup` |
| 版本一直是 1.0.0 | 用 `.\build_installer.bat 1.2.0` 显式指定 |
| PowerShell 提示无法识别 bat | 写成 `.\build_installer.bat 1.2.0`（不要漏掉 `.\`） |
| 双击 bat 一闪而过 | 看报错；失败时脚本会 `pause` |
| PowerShell 报 Unexpected token / 中文乱码 | 拉取最新后再运行 `.\build_installer.bat 1.2.0` |

## 其他

- 可选创建桌面快捷方式
- 卸载不会删除 `%USERPROFILE%\.novel_listener` 里的听书进度
